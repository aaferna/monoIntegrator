...
async function executeSQLS(database, sql) {
	try {
		...
		if (!selectedDB) {
			log(
				'error',
				`El Conector "${database}" no es correcto o no existe`,
				'SQL',
			);
			return { status: 0 };
		}
		...
		if (error.code === 'PROTOCOL_CONNECTION_LOST') {
			...
			log(
				'warning',
				`Se perdió la conexión con la base de datos. Reintentando la consulta...`,
				'SQL',
			);
			...
			log(
				'warning',
				`Se alcanzó el límite de conexiones permitido. Esperando antes de reintentar...`,
				'SQL',
			);
...
async function executeSQLPool(database, queries) {
	try {
		const selectedDB = dbconect.find(item => item.name === database);
		...
		for (const query of queries) {
			try {
				...
				if (error.code === 'PROTOCOL_CONNECTION_LOST') {
					...
					await new Promise(resolve => setTimeout(resolve, 5000));
					return executeSQL(database, queries);
				} else if (error.code === 'POOL_ENQUEUELIMIT') {
					// Si se alcanza el límite de la pool, se muestra una advertencia y se espera antes de reintentar
					log(
						'warning',
		...
		log('error', `Error en la función executeSQL :: ${error}`, 'SQL');
		return { status: 0 };
	}
}

exports.executeSQLS = executeSQLS;
exports.executeSQLPool = executeSQLPool;
